    ...
    public partial class frmSalesRecord : Form
    {
        ...
        private void cmbCustomerName_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                con = new SqlConnection(cs);
                con.Open();
                cmd = new SqlCommand("SELECT (invoiceNo) as [Invoice No],(InvoiceDate) as [Invoice Date],(Sales.CustomerID) as [Customer ID],(CustomerName) as [Customer Name],(GrandTotal) as [Grand Total],(TotalPayment) as [Total Payment],(PaymentDue) as [Payment Due] from Sales,Customer where Sales.CustomerID=Customer.CustomerID and Customername='" + cmbCustomerName.Text + "' order by CustomerName,InvoiceDate", con);
                var myDA = new SqlDataAdapter(cmd);
                var myDataSet = new DataSet();
                myDA.Fill(myDataSet, "Sales");
                myDA.Fill(myDataSet, "Customer");
        ...
        private void button10_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show("Please select Customer name", string.Empty, MessageBoxButtons.OK, MessageBoxIcon.Error);
